...
class HookValidator:
    ...
    def _run_hook(self, hook_name: str, context: Dict = None, timeout: int = 10) -> Tuple[bool, str, float]:
        """执行Hook并返回结果"""
    ...
    def validate_hook(self, hook_name: str, test_scenarios: List[Dict] = None) -> Dict:
        ...
        if test_scenarios:
            ...
            for scenario in test_scenarios:
                ...
                output_valid = True
                if expected:
                    output_valid = any(exp.lower() in output.lower() for exp in expected)
            ...
            hook_result['performance'] = {
                ...
                'average_duration': total_duration / len(test_scenarios) if test_scenarios else 0,
                'max_duration': max(s['duration'] for s in hook_result['scenarios']),
                'successful_scenarios': successful_scenarios,
                'total_scenarios': len(test_scenarios),
                'success_rate': successful_scenarios / len(test_scenarios) * 100 if test_scenarios else 0
            }
    ...
    def _generate_recommendations(self, results: Dict) -> List[str]:
        ...
        if summary['not_found'] > 0:
            recommendations.append(f"发现{summary['not_found']}个Hook文件不存在，建议检查.claude/hooks目录")

        # 性能建议
        if 'performance_overview' in results:
            perf = results['performance_overview']
            if perf['max_hook_duration'] > 5.0:
                recommendations.append(f"部分Hook执行时间过长(最长{perf['max_hook_duration']:.2f}s)，建议优化性能")

            if perf['average_hook_duration'] > 2.0:
                recommendations.append(f"Hook平均执行时间较长({perf['average_hook_duration']:.2f}s)，建议整体优化")

        # 具体Hook建议
        critical_hooks_missing = []
        critical_hooks = ['smart_agent_selector', 'error_handler', 'performance_monitor']
        ...
        if non_executable:
            recommendations.append(f"以下Hook文件存在但不可执行: {', '.join(non_executable)}，请运行chmod +x")
    ...
    def print_summary(self, results: Dict) -> None:
        ...
        for hook_name, result in results['hook_results'].items():
            ...
            print(f"   {icon} {hook_name:<30} ({success_rate:>5.1f}%) {duration:>6.3f}s")
...
def main():
    ...
    try:
        ...
        # 返回退出代码
        success_rate = (results['summary']['passed'] / max(1, results['summary']['total_hooks'])) * 100
        if success_rate >= 80:
            print("🎉 Hook验证通过！")
            sys.exit(0)
