...
class PerformanceTestSuite:
    ...
    def add_metric(self, name: str, value: float, unit: str, category: str,
                   iteration: int = 0, additional_data: Optional[Dict] = None):
        """添加性能指标"""
    ...
    def test_startup_performance(self, iterations: int = 50) -> Dict[str, Any]:
        ...
        orchestrator_stats = {
            ...
            'std': statistics.stdev(orchestrator_times) if len(orchestrator_times) > 1 else 0,
        ...
        self.add_metric("orchestrator_startup_avg", orchestrator_stats['avg'], "ms", "startup")

        # 基准对比（假设5.0版本的基准值）
        baseline_engine = 3.0  # 假设5.0版本的基准启动时间3ms
        baseline_orchestrator = 1.0  # 假设5.0版本的基准启动时间1ms

        engine_improvement = ((baseline_engine - engine_stats['avg']) / baseline_engine) * 100
        orchestrator_improvement = ((baseline_orchestrator - orchestrator_stats['avg']) / baseline_orchestrator) * 100

        print(f"  LazyWorkflowEngine: 平均 {engine_stats['avg']:.3f}ms (改进 {engine_improvement:.1f}%)")
        print(f"  LazyAgentOrchestrator: 平均 {orchestrator_stats['avg']:.3f}ms (改进 {orchestrator_improvement:.1f}%)")
        ...
        return {
            ...
            "target_met": engine_improvement >= self.performance_targets['startup_speed_improvement']
        }

    def test_concurrency_performance(self, max_workers: int = 20,
                                   tasks_per_worker: int = 10) -> Dict[str, Any]:
        """测试并发处理性能"""
        ...
        if completed_tasks:
            ...
            concurrency_stats = {
                ...
                'success_rate': (len(completed_tasks) / (max_workers * tasks_per_worker)) * 100
            }

            # 记录指标
            self.add_metric("concurrency_throughput", throughput, "tasks/sec", "concurrency")
            self.add_metric("avg_task_execution", concurrency_stats['avg_task_time'], "ms", "concurrency")
            ...
            return {
                ...
                "target_met": throughput_improvement >= self.performance_targets['concurrency_improvement']
            }
        else:
    ...
    def test_memory_efficiency(self, cycles: int = 100) -> Dict[str, Any]:
        ...
        cleanup_efficiency = ((len(objects_created) - alive_objects) / len(objects_created)) * 100
        ...
        return {
            ...
            "target_met": cleanup_efficiency >= 95 and memory_growth < initial_memory * 0.1
        }
    ...
    def test_cpu_efficiency(self, duration: int = 30) -> Dict[str, Any]:
        ...
        self.add_metric("operations_per_second", operations_per_second, "ops/sec", "cpu")
    ...
    def run_stress_test(self, duration: int = 60) -> Dict[str, Any]:
        ...
        error_rate = (stress_results['errors'] / total_operations * 100) if total_operations > 0 else 0

        if stress_results['response_times']:
            avg_response = statistics.mean(stress_results['response_times'])
            p95_response = sorted(stress_results['response_times'])[int(len(stress_results['response_times']) * 0.95)]
        else:
            avg_response = 0
            p95_response = 0
    ...
    def run_full_test_suite(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # 2. 并发处理测试
            test_results["concurrency"] = self.test_concurrency_performance(max_workers=15, tasks_per_worker=20)
...
def print_performance_summary(report: Dict[str, Any]):
    ...
    print(f"Claude Enhancer可用: {'是' if test_summary.get('claude_enhancer_available', False) else '否'}")
