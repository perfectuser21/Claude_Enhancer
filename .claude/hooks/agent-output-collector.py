...
class AgentOutputCollector:
    def __init__(self):
        self.output_dir = Path("' + CLAUDE_ENHANCER_HOME + '/.claude_enhancer/agent_outputs")
    ...
    def collect_and_summarize(self, agent_info):
        ...
        for agent in agent_info["agents"]:
            ...
            summary["compressed_outputs"][agent].append(
                {"timestamp": agent_info["timestamp"], "key_points": key_info}
            )

            # 限制每个Agent的历史记录
            if len(summary["compressed_outputs"][agent]) > 3:
                summary["compressed_outputs"][agent] = summary["compressed_outputs"][
                    agent
                ][-3:]
