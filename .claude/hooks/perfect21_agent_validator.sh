#!/bin/bash
# Perfect21 Agent Validator Hook
# 验证Agent选择是否符合Perfect21规则
...
# 配置文件路径
RULES_FILE="/home/xx/dev/Perfect21/rules/perfect21_rules.yaml"
...
if [ "$AGENT_COUNT" -lt 3 ]; then
    echo "❌ Perfect21 规则违反：Agent数量不足" >&2
    echo "" >&2
    echo "📊 当前状态：" >&2
...
if [ "$TASK_TYPE" != "general" ]; then
    ...
    if [ -n "$REQUIRED_AGENTS" ]; then
        ...
        if [ -n "$MISSING_AGENTS" ]; then
            echo "⚠️ Perfect21 规则提醒：Agent组合不完整" >&2
            echo "" >&2
            echo "📊 任务类型：$TASK_TYPE" >&2
            echo "" >&2
            echo "🔍 当前使用的Agents：" >&2
...
if echo "$INPUT" | grep -q "function_calls" && [ "$AGENT_COUNT" -gt 1 ]; then
    ...
    if [ -n "$TASK_POSITIONS" ]; then
        ...
        if [ "$DISTANCE" -gt 50 ] && [ "$AGENT_COUNT" -gt 1 ]; then
            echo "⚠️ Perfect21 提醒：请确保所有Agent在同一个function_calls块中并行执行" >&2
        fi
    fi
fi

# 记录执行日志
LOG_FILE="/tmp/perfect21_agent_log.txt"
