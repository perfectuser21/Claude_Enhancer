...
class AgentOutputSummarizer:
    ...
    def _extract_metrics(self, output: str) -> Dict[str, Any]:
        ...
        # 提取性能改进
        perf_match = re.search(r'(\d+)%\s+(?:improvement|faster|reduction)', output, re.IGNORECASE)
        if perf_match:
    ...
    def generate_summary_report(self) -> str:
        ...
        report.append(f"- **总输出大小**: {self._format_size(self.execution_stats['total_output_size'])}")
        ...
        for idx, (agent_type, data) in enumerate(sorted(self.agents_data.items()), 1):
            ...
            report.append(f"- **输出大小**: {self._format_size(data.get('output_size', 0))}")
        ...
        total_loc = sum(d.get('metrics', {}).get('lines_of_code', 0) for d in self.agents_data.values())
        total_files = sum(d.get('metrics', {}).get('files_created', 0) for d in self.agents_data.values())
