...
if echo "$INPUT_LOWER" | grep -qE "$DEV_KEYWORDS"; then
    if [[ "${CE_SILENT_MODE:-false}" != "true" ]]; then
        echo "╔═══════════════════════════════════════════════════╗" >&2
        echo "║   🎯 Claude Enhancer 智能工作流引导系统 🎯      ║" >&2
        echo "╚═══════════════════════════════════════════════════╝" >&2
        echo "" >&2
        ...
        echo "" >&2
        echo "📋 强制执行的7-Phase工作流：" >&2
        echo "   Phase 1: 创建分支 → git checkout -b feature/xxx" >&2
        echo "   Phase 2: 探索发现 → 技术spike，可行性验证" >&2
        echo "   Phase 3: 规划+架构 → 使用Task调用分析Agent" >&2
        echo "   Phase 4: 并行开发 → 【关键】多Agent并行实现" >&2
        echo "   Phase 5: 本地测试 → 运行测试套件（质量门禁1）" >&2
        echo "   Phase 6: 代码审查 → 创建PR（质量门禁2）" >&2
        echo "   Phase 7: 发布+监控 → 完成任务" >&2
        echo "" >&2
        echo "⚠️  重要提醒：" >&2
        echo "   • 不要直接写代码！使用Task工具调用Agent" >&2
        echo "   • 所有Agent必须在同一个function_calls块中并行调用" >&2
        echo "   • 这是Claude Enhancer的核心要求，不是可选项" >&2
        echo "" >&2
        echo "💡 正确的第一步：" >&2
        ...
        if echo "$INPUT_LOWER" | grep -qE "压力测试|性能优化|架构|重构|全栈"; then
            echo "[TaskDetect] 🔴 复杂开发任务 (需6-8 Agents)" >&2
        else
            echo "[TaskDetect] 🟡 标准开发任务 (需4-6 Agents)" >&2
        fi
    fi
