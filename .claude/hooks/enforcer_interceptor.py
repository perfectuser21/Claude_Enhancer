#!/usr/bin/env python3
"""
Perfect21 Enforcer Interceptor - Claude Code执行拦截器

这个拦截器在Claude Code实际执行任何工具之前运行，
能够真正阻止不符合Perfect21规则的执行，并强制重定向到正确的方案。
"""
...
sys.path.insert(0, '/home/xx/dev/Perfect21/core/enforcer')
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('/tmp/perfect21_interceptor.log'),
        logging.StreamHandler(sys.stderr)
    ]
)
logger = logging.getLogger('perfect21.interceptor')
...
"""
        ...
        return f"Perfect21 处理结果: {intercept_result.get('message', '未知状态')}"
    ...
    if event_type == 'PreToolUse':
        ...
        if result.get('action') == 'redirect':
            ...
            if modified_execution:
                with open('/tmp/perfect21_modified_execution.txt', 'w') as f:
                    f.write(modified_execution)
                print(f"\n修正后的执行计划已保存到: /tmp/perfect21_modified_execution.txt")

    elif event_type == 'PostToolUse':
        # 后置处理
        print(f"Perfect21 Post-tool processing for {tool_name}")

    else:
        # 其他事件
        print(f"Perfect21 处理事件: {event_type}")
