...
main() {
    ...
    # 只在Phase1 (Discovery)完成后、Phase2 (Implementation)开始前触发
    if [[ "$current_phase" == "Phase1" ]] && is_phase1_3_completed; then
        # 检查是否已评估
        if ! is_impact_assessed; then
            echo "🚨 Phase 1 (Discovery)完成后必须进行Impact Radius评估！"
            echo "   这将自动计算任务的风险、复杂度和影响范围"
            echo "   并推荐最优的Agent数量（0/3/6 agents）"
            echo ""

            # 记录enforcement日志
            echo "[$(date +'%F %T')] [impact_assessment_enforcer.sh] [BLOCK] Impact Assessment not performed after Phase 1" >> "$LOG_FILE"
            ...
            if [[ -f "$PROJECT_ROOT/.claude/hooks/smart_agent_selector.sh" ]]; then
                ...
                if bash "$PROJECT_ROOT/.claude/hooks/smart_agent_selector.sh"; then
                    ...
                    echo "[$(date +'%F %T')] [impact_assessment_enforcer.sh] [AUTO_FIX] Impact Assessment auto-executed successfully (Phase 1)" >> "$LOG_FILE"
                    exit 0
                else
