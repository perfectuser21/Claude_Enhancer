...
test_cleanup_performance() {
    ...
    bash /home/xx/dev/Perfect21/.claude/scripts/performance_optimized_cleanup.sh 5 >/dev/null 2>&1
...
test_agent_selector_performance() {
    ...
    local start_time=$(date +%s%N)
    echo "$test_input" | bash /home/xx/dev/Perfect21/.claude/hooks/smart_agent_selector.sh >/dev/null 2>&1
    local end_time=$(date +%s%N)
...
---
*性能监控报告 - Perfect21 Performance Team*
EOF
...
real_time_monitor() {
    ...
    while true; do
        clear
        echo -e "${BLUE}📊 Perfect21 实时性能监控${NC}"
        echo "========================================"
...
# Main function
main() {
    echo -e "${BLUE}🚀 Perfect21 性能监控系统${NC}"
    echo "========================================"
    echo ""
