...
benchmark_agent_selector() {
    ...
    for i in $(seq 1 $ITERATIONS); do
        local time_result=$(measure_time "echo '$test_input' | bash /home/xx/dev/Perfect21/.claude/hooks/smart_agent_selector.sh" "智能Agent选择")
        times+=("$time_result")
...
benchmark_cleanup_scripts() {
    ...
    # 测试原始清理脚本
    if [ -f "/home/xx/dev/Perfect21/.claude/scripts/cleanup.sh" ]; then
        echo "  🔄 测试原始cleanup.sh"
        ...
        for i in $(seq 1 $ITERATIONS); do
            ...
            local time_result=$(measure_time "bash /home/xx/dev/Perfect21/.claude/scripts/cleanup.sh 5" "原始清理")
    ...
    # 测试优化版清理脚本
    if [ -f "/home/xx/dev/Perfect21/.claude/scripts/performance_optimized_cleanup.sh" ]; then
        ...
        for i in $(seq 1 $ITERATIONS); do
            ...
            local time_result=$(measure_time "bash /home/xx/dev/Perfect21/.claude/scripts/performance_optimized_cleanup.sh 5" "优化清理")
...
benchmark_cache_system() {
    ...
    for i in {1..10}; do
        ...
        bash /home/xx/dev/Perfect21/.claude/hooks/smart_agent_selector.sh >/dev/null 2>&1
    done
...
---
*基准测试完成 - Perfect21 Performance Team*
EOF
...
# 主执行函数
main() {
    echo -e "${BLUE}🚀 Perfect21 性能基准测试${NC}"
    echo "========================================"
    echo ""
