...
class ConfigMigrator:
    ...
    def _setup_logging(self) -> logging.Logger:
        ...
        logger = logging.getLogger("perfect21.migration")
    ...
    def migrate_all(self, backup: bool = True) -> Dict[str, Any]:
        ...
        for legacy_file in legacy_files:
            try:
                ...
                unified_config = self._merge_migrated_data(unified_config, migrated_data)
    ...
    def _create_base_config(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "integrations": {
                "git_hooks": {"enabled": True, "auto_install": False}
            },
            "notifications": {
                "enabled": True,
                "formats": ["emoji"]
    ...
    def _migrate_legacy_config(self, file_path: Path, config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _migrate_settings_json(self, config: Dict[str, Any]) -> Dict[str, Any]:
            ...
            for event, hook_list in hooks.items():
                if event in ['PreToolUse', 'PostToolUse', 'UserPromptSubmit']:
                    ...
                    for hook in hook_list:
                        migrated_hook = {
                            'name': hook.get('description', 'Unknown').replace(' ', '_').lower(),
        ...
        if 'environment' in config:
            ...
            migrated['agents'] = {
                'execution': {
                    'enforce_parallel': env.get('ENFORCE_PARALLEL', 'true').lower() == 'true',
                    'timeout': 30000
                },
                'selection': {
            ...
            if 'MIN_AGENTS' in env:
                migrated['agents']['selection']['minimum_agents'] = int(env['MIN_AGENTS'])
    ...
    def _migrate_enhancer_config(self, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if 'hooks' in config:
            ...
            migrated['hooks'] = {
                'enabled': True,
                'enforcement_level': 'strict' if hooks.get('agent_validator', {}).get('strict') else 'warning'
            }
        ...
        if 'task_types' in config:
            ...
            for task_name, task_config in config['task_types'].items():
                migrated['task_types'][task_name] = {
                    'keywords': task_config.get('keywords', []),
                    'required_agents': task_config.get('required', []) + task_config.get('recommended', []),
        ...
        if 'task_types' in config:
            ...
            for task_name, task_config in config['task_types'].items():
                migrated['task_types'][task_name] = {
                    ...
                    'complexity': self._determine_complexity(task_config.get('minimum_count', 3))
                }
    ...
    def _merge_migrated_data(self, base_config: Dict[str, Any], migrated_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        def deep_merge(base: Dict, override: Dict) -> Dict:
            ...
            for key, value in override.items():
                if key in result and isinstance(result[key], dict) and isinstance(value, dict):
                    ...
                    if all(isinstance(item, (str, int, float, bool)) for item in combined):
...
def main():
    ...
    parser.add_argument("--discover-only", action="store_true", help="Only discover legacy files")
