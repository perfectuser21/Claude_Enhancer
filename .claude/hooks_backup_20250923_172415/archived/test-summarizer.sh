...
echo ""
echo "🔄 执行汇总..."
python3 ${CLAUDE_ENHANCER_HOME}/.claude/hooks/agent-summarizer.py /tmp/test_agent_output.json > /tmp/summary_result.json
...
echo '<function_calls><invoke name="Task"><parameter name="subagent_type">backend-architect</parameter></invoke></function_calls>' | python3 ${CLAUDE_ENHANCER_HOME}/.claude/hooks/agent-output-collector.py
