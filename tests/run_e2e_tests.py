...
示例:
  ...
  python run_e2e_tests.py --stress           # 压力测试模式
    ...
    parser.add_argument("--stress", action="store_true", help="压力测试模式（增加测试强度）")

    parser.add_argument("--parallel", type=int, default=4, help="并发测试线程数（默认: 4）")

    parser.add_argument("--timeout", type=int, default=30, help="单个测试超时时间（秒，默认: 30）")
    ...
    if args.mode == "full":
        ...
        report = framework.generate_test_report(
            sum(r.duration for r in framework.test_results)
        )
...
def print_report(report, verbose=False):
    ...
    if report.get("phase_breakdown"):
        ...
        for phase, stats in report["phase_breakdown"].items():
            ...
            print(
                f"   📌 {phase}: {stats['passed']}/{stats['total']} ({phase_success_rate:.1f}%)"
            )
    ...
    if verbose and report.get("detailed_results"):
        ...
        for result in report["detailed_results"][:20]:  # 只显示前20个
            ...
            print(
                f"   {status_icon} {result['name']} ({result.get('duration', 0):.3f}s)"
            )
