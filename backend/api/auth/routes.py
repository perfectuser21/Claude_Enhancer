...
from .models import (
    ...
    RefreshTokenRequest, RefreshTokenResponse, MFAVerificationRequest,
    MFAEnableRequest, MFASetupResponse, PasswordChangeRequest,
    PasswordResetRequest, UserProfile, StandardResponse, ErrorResponse
)
from .dependencies import (
    get_client_info, get_current_user, get_current_active_user,
    get_user_service, get_jwt_manager, get_mfa_service,
    require_permissions, rate_limit_check
)
    ...
    try:
        ...
        if hasattr(current_user, '_token_claims'):
            await jwt_manager.revoke_token(
                jti=current_user._token_claims.jti,
                reason="user_logout"
            )
...
async def enable_mfa(
    ...
    try:
        ...
        if current_user.mfa_enabled:
            existing_methods = await user_service.get_user_mfa_methods(str(current_user.id))
