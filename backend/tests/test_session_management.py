...
class Session:
    ...
    def __init__(self, id: str = None, user_id: str = None, session_id: str = None,
                 token_hash: str = None, status: SessionStatus = SessionStatus.ACTIVE,
                 expires_at: datetime = None, last_activity_at: datetime = None,
                 device_type: DeviceType = DeviceType.UNKNOWN, device_id: str = None,
                 device_name: str = None, user_agent: str = None, ip_address: str = None,
                 csrf_token: str = None, is_trusted_device: bool = False,
                 risk_score: int = 0, metadata: Dict = None, **kwargs):
    ...
    @classmethod
    def create_session(cls, user_id: str, token: str, expires_in_seconds: int = 3600,
                      device_type: DeviceType = DeviceType.UNKNOWN, ip_address: str = None,
                      user_agent: str = None, **kwargs) -> 'Session':
        """创建新会话"""
        now = datetime.utcnow()
...
class RefreshToken:
    ...
    def __init__(self, id: str = None, session_id: str = None, token_hash: str = None,
                 token_family: str = None, expires_at: datetime = None,
                 used_count: int = 0, max_uses: int = 1, last_used_at: datetime = None,
                 is_revoked: bool = False, **kwargs):
        self.id = id or str(uuid.uuid4())
    ...
    @classmethod
    def create_refresh_token(cls, session_id: str, token: str, 
                           expires_in_days: int = 30, max_uses: int = 1) -> 'RefreshToken':
        """创建刷新令牌"""
        return cls(
...
class SessionManager:
    ...
    async def create_session(self, user_id: str, token: str, device_info: Dict = None,
                           ip_address: str = None, user_agent: str = None,
                           expires_in_seconds: int = 3600, **kwargs) -> Session:
        """创建新会话"""
        # 创建会话
        session = Session.create_session(
            ...
            device_type=device_info.get("device_type", DeviceType.UNKNOWN) if device_info else DeviceType.UNKNOWN,
            ip_address=ip_address,
            user_agent=user_agent,
            **kwargs
        )
    ...
    async def create_refresh_token(self, session_id: str, token: str,
                                 expires_in_days: int = 30, max_uses: int = 1) -> RefreshToken:
        """创建刷新令牌"""
    ...
    async def get_session_by_device_fingerprint(self, device_fingerprint: str) -> Optional[Session]:
        """根据设备指纹获取会话"""
    ...
    async def get_session_statistics(self, user_id: str = None) -> Dict[str, Any]:
        ...
        stats = {
            ...
            "revoked_sessions": len([s for s in sessions if s.status == SessionStatus.REVOKED]),
            "device_types": {},
        ...
        if sessions:
            total_duration = sum(
                ...
                for s in sessions if s.last_activity_at
            )
...
class TestSessionCreation:
    ...
    @pytest.mark.asyncio
    async def test_create_session_with_device_fingerprint(self, session_manager, user_id, device_info):
        """测试创建带设备指纹的会话"""
...
class TestSessionVerification:
    ...
    async def test_verify_revoked_session(self, session_manager, active_session):
        """测试验证已撤销的会话"""
        # 撤销会话
        await session_manager.revoke_session(active_session.session_id, "test_revocation")
    ...
    @pytest.mark.asyncio
    async def test_verify_session_updates_activity(self, session_manager, active_session):
        """测试验证会话时更新活动时间"""
...
class TestSessionExtension:
    ...
    @pytest.mark.asyncio
    async def test_extend_session_updates_activity(self, session_manager, active_session):
        """测试延长会话时更新活动时间"""
        original_activity = active_session.last_activity_at
...
class TestRefreshTokenManagement:
    ...
    @pytest.mark.asyncio
    async def test_use_refresh_token_multiple_times(self, session_manager, active_session):
        """测试多次使用刷新令牌"""
...
class TestDeviceFingerprintManagement:
    ...
    @pytest.mark.asyncio
    async def test_get_session_by_device_fingerprint(self, session_manager, device_info):
        """测试根据设备指纹获取会话"""
        # 创建带设备指纹的会话
        session = await session_manager.create_session(
    ...
    async def test_device_fingerprint_uniqueness(self, session_manager):
        ...
        # 最后一个会话应该覆盖前一个
        found_session = await session_manager.get_session_by_device_fingerprint(fingerprint)
...
class TestSessionStatistics:
    ...
    async def test_get_user_sessions(self, session_manager):
        ...
        # 获取活跃会话
        active_sessions = await session_manager.get_user_sessions(user_id, active_only=True)
        assert len(active_sessions) == 2
        
        # 获取所有会话
        all_sessions = await session_manager.get_user_sessions(user_id, active_only=False)
...
class TestSessionManagerIntegration:
    ...
    async def test_complete_session_lifecycle(self, session_manager):
        ...
        # 4. 延长会话
        extend_result = await session_manager.extend_session(
            session.session_id, 7200
        )
