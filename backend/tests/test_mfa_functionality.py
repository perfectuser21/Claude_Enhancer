...
class QRCodeGenerator:
    ...
    @staticmethod
    def generate_totp_qr(secret: str, user_email: str, issuer: str = "Perfect21",
...
class MFAService:
    ...
    async def setup_mfa(self, user_id: str, method: str, phone_number: str = None,
    ...
    async def generate_mfa_challenge(self, user_id: str, ip_address: str = None,
    ...
    async def verify_mfa_challenge(self, mfa_token: str, verification_code: str,
                                 ip_address: str = None, device_info: Dict = None) -> Dict[str, Any]:
        """验证MFA挑战"""
        # 检查挑战是否存在
    ...
    async def enable_mfa_method(self, user_id: str, method: str, verification_code: str = None) -> bool:
        """启用MFA方法"""
        if user_id not in self.user_mfa_settings:
            return False
    ...
    async def trust_device(self, user_id: str, device_fingerprint: str, 
                         trust_duration_days: int = 30) -> None:
        """信任设备"""
        if user_id not in self.trusted_devices:
        ...
        now = datetime.utcnow()
        self.trusted_devices[user_id] = [
            device for device in self.trusted_devices[user_id]
...
"""
    ...
    async def test_enable_totp_with_verification(self, mfa_service, user_id):
        ...
        # 启用TOTP
        result = await mfa_service.enable_mfa_method(user_id, MFAMethod.TOTP, verification_code)
    ...
    async def test_enable_totp_with_wrong_verification(self, mfa_service, user_id):
        ...
        # 尝试启用TOTP
        result = await mfa_service.enable_mfa_method(user_id, MFAMethod.TOTP, wrong_code)
...
"""
    ...
    async def test_setup_sms_success(self, mfa_service, user_id, phone_number):
        """测试成功设置SMS MFA"""
        # 执行
        result = await mfa_service.setup_mfa(user_id, MFAMethod.SMS, phone_number=phone_number)
        ...
        assert mfa_service.user_mfa_settings[user_id][MFAMethod.SMS]["phone_number"] == phone_number
    ...
    @pytest.mark.asyncio
    async def test_sms_challenge_generation_and_verification(self, mfa_service, user_id, phone_number):
        """测试SMS挑战生成和验证"""
        # 设置并启用SMS
...
"""
    ...
    async def test_backup_code_verification(self, mfa_service, user_id):
        ...
        # 检查备用码是否被删除
        remaining_codes = [
            code for code in mfa_service.backup_codes[user_id]
...
"""
    ...
    async def test_trust_device(self, mfa_service, user_id, device_fingerprint):
        """测试信任设备"""
        # 信任设备
        await mfa_service.trust_device(user_id, device_fingerprint, trust_duration_days=30)
    ...
    @pytest.mark.asyncio
    async def test_device_trust_expiration(self, mfa_service, user_id, device_fingerprint):
        """测试设备信任过期"""
        # 信任设备，但设置过去的过期时间
        await mfa_service.trust_device(user_id, device_fingerprint, trust_duration_days=-1)
...
"""
    ...
    async def test_challenge_expiration(self, mfa_service, user_id):
        ...
        # 模拟挑战过期
        mfa_service.active_challenges[challenge_token]["expires_at"] = \
...
"""
    ...
    async def test_complete_mfa_flow(self, mfa_service, user_id):
        ...
        # 5. 使用TOTP验证
        new_code = totp.now()
        verify_result = await mfa_service.verify_mfa_challenge(challenge_token, new_code)
