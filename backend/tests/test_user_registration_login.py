...
class UserService:
    ...
    async def create_user(
        self, user_data: Dict[str, Any], registration_context: Dict[str, Any] = None
    ) -> User:
        """创建用户"""
    ...
    async def authenticate_user(
        self, email: str, password: str, login_context: Dict[str, Any] = None
    ) -> Optional[User]:
        """用户认证"""
...
class TestUserRegistration:
    ...
    async def test_register_user_success(
        self, user_service, valid_user_data, registration_context
    ):
        """测试成功注册用户"""
        # 执行
    ...
    async def test_register_user_without_optional_fields(
        self, user_service, registration_context
    ):
        """测试注册用户时省略可选字段"""
        minimal_data = {
    ...
    async def test_email_verification_invalid_token(self, user_service):
        """测试无效验证令牌"""
        invalid_tokens = ["invalid-token", str(uuid.uuid4()), "", None]  # 不存在的令牌
    ...
    async def test_email_verification_expired_token(
        ...
        # 模拟令牌过期
        user_service.email_verification_tokens[verification_token][
            "expires_at"
...
class TestUserLogin:
    ...
    async def test_login_account_lockout(
        self, user_service, active_user, login_context
    ):
        """测试账户锁定机制"""
        # 连续失败登录5次
        for i in range(5):
    ...
    async def test_login_email_case_insensitive(
        ...
        for email in email_variations:
            authenticated_user = await user_service.authenticate_user(
                email=email, password="SecurePassword123!", login_context=login_context
            )
    ...
    @pytest.mark.asyncio
    async def test_login_multiple_sessions(
        self, user_service, active_user, login_context
    ):
        """测试多次登录计数"""
        login_count = 5
...
class TestPasswordManagement:
    ...
    async def test_request_password_reset(self, user_service, test_user):
        ...
        result = await user_service.request_password_reset(
            email="password_test@example.com", ip_address="192.168.1.100"
        )
    ...
    async def test_request_password_reset_nonexistent_user(self, user_service):
        """测试不存在用户的密码重置请求"""
        result = await user_service.request_password_reset(
            email="nonexistent@example.com", ip_address="192.168.1.100"
        )
...
class TestUserPermissions:
    ...
    async def regular_user(self, user_service):
        user_data = {"email": "regular@example.com", "password": "Password123!"}
        user = await user_service.create_user(user_data)
        user.status = "ACTIVE"
        user.role = "user"
        return user
    ...
    async def admin_user(self, user_service):
        user_data = {"email": "admin@example.com", "password": "AdminPassword123!"}
        user = await user_service.create_user(user_data)
        user.status = "ACTIVE"
        user.role = "admin"
        return user
