...
class MFAService:
    ...
    @monitor_function("mfa")
    async def generate_mfa_challenge(self, user_id: str, ip_address: str, 
                                   preferred_method: str = None) -> str:
        """生成多因子认证挑战"""
        try:
            # 获取用户的MFA方法
            ...
            # 选择认证方法
            method = preferred_method if preferred_method in available_methods else available_methods[0]
    ...
    async def verify_mfa_challenge(self, mfa_token: str, verification_code: str, 
        ...
        try:
            ...
            if challenge.method_type == 'totp':
                is_valid = await self._verify_totp_code(challenge.user_id, verification_code)
                ...
                is_valid = await self._verify_backup_code(challenge.user_id, verification_code)
            ...
            if is_valid:
                ...
                # 更新最后使用时间
                await self._update_method_last_used(challenge.user_id, challenge.method_type)
