...
class UserService:
    ...
    async def set_dependencies(self, message_publisher: MessagePublisher, 
                             password_service: PasswordService, email_service: EmailService):
        """设置依赖服务"""
    ...
    async def create_user(self, user_data: Dict[str, Any], 
                        registration_context: Dict[str, Any]) -> User:
        """创建新用户"""
        async with get_async_session() as session:
            try:
                ...
                # 生成邮箱验证令牌
                verification_token = await self._generate_verification_token(str(user.id))
        ...
        async with get_async_session() as session:
            try:
                ...
                if not user.is_active:
                    await self._log_failed_login(email, "account_inactive", login_context)
                    return None
                ...
                if not verification_result.is_valid:
                    ...
                    await self._log_failed_login(email, "invalid_password", login_context)
                    return None
    ...
    async def get_user_permissions(self, user_id: str) -> List[str]:
        ...
        try:
            ...
            await self.redis_client.setex(
                cache_key, 
                settings.CACHE_TTL_USER_PERMISSIONS, 
                ','.join(permissions)
            )
    ...
    async def reset_password(self, reset_token: str, new_password: str, 
        ...
        try:
            ...
            strength_result = await self.password_service.validate_password_strength(new_password)
            if not strength_result.is_strong:
                raise ValueError(f"密码强度不足: {', '.join(strength_result.feedback)}")
    ...
    @monitor_function("user")
    async def trust_device(self, user_id: str, device_fingerprint: str, ip_address: str):
        """信任设备"""
        async with get_async_session() as session:
            try:
    ...
    async def _record_failed_attempt(self, user_id: str, login_context: Dict[str, Any]):
        ...
        if attempt_count >= settings.ACCOUNT_LOCKOUT_ATTEMPTS:
            ...
            lockout_key = f"account_lockout:{user_id}"
            await self.redis_client.setex(lockout_key, lockout_duration, str(lockout_data))
    ...
    async def _log_failed_login(self, email: str, reason: str, login_context: Dict[str, Any]):
        """记录失败登录"""
        if self.message_publisher:
            await self.message_publisher.publish_message(
                message_type=MessageType.SECURITY_ALERT,
    ...
    async def _handle_password_risk_factors(self, user: User, risk_factors: List[str], 
    ...
    async def _generate_reset_token(self, user_id: str) -> str:
        ...
        await self.redis_client.setex(
            token_key, 
            settings.EMAIL_RESET_PASSWORD_TTL, 
            user_id
        )
