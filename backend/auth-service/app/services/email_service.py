...
class EmailService:
    ...
    def _load_default_templates(self):
        ...
        self._templates = {
            'verification': EmailTemplate(
                ...
                {{ app_name }}团队
                ''',
                variables=['first_name', 'app_name', 'verification_url', 'expiry_hours', 'current_year']
            ),
            ...
            'password_reset': EmailTemplate(
                ...
                ''',
                variables=['first_name', 'app_name', 'reset_url', 'expiry_hours', 'current_year']
            ),
            ...
            'security_alert': EmailTemplate(
                ...
                ''',
                variables=['first_name', 'app_name', 'alert_message', 'event_time', 'ip_address', 'location', 'device_info', 'recommended_actions', 'current_year']
            )
        }

    @monitor_function("email")
    async def send_verification_email(self, email: str, first_name: str, 
                                    verification_token: str) -> EmailSendResult:
        """发送邮箱验证邮件"""
        try:
    ...
    @monitor_function("email")
    async def send_password_reset_email(self, email: str, first_name: str, 
                                      reset_token: str) -> EmailSendResult:
        """发送密码重置邮件"""
        try:
            ...
            return EmailSendResult(
                success=False,
                error=f"发送密码重置邮件失败: {str(e)}"
            )

    @monitor_function("email")
    async def send_security_alert_email(self, email: str, first_name: str, 
                                      alert_type: str, details: Dict[str, Any]) -> EmailSendResult:
        """发送安全警告邮件"""
        try:
            alert_messages = {
    ...
    async def send_custom_email(self, message: EmailMessage) -> EmailSendResult:
        ...
        try:
            ...
            if not rate_limit_check['allowed']:
                return EmailSendResult(
                    success=False,
                    error=f"发送频率超限: {rate_limit_check['message']}"
                )
    ...
    async def _send_template_email(self, template_id: str, to_email: str, 
                                 to_name: str, variables: Dict[str, Any], 
                                 priority: int = 5) -> EmailSendResult:
        """使用模板发送邮件"""
        ...
        html_content = self.template_env.from_string(template.html_content).render(**variables)
        text_content = None
        
        if template.text_content:
            text_content = self.template_env.from_string(template.text_content).render(**variables)
    ...
    async def _check_rate_limit(self, email: str) -> Dict[str, Any]:
        ...
        try:
            # 每小时限制
            hourly_key = f"email_rate_limit:hourly:{email}:{datetime.now().strftime('%Y%m%d%H')}"
            ...
            # 每日限制
            daily_key = f"email_rate_limit:daily:{email}:{datetime.now().strftime('%Y%m%d')}"
    ...
    @monitor_function("email")
    async def get_email_stats(self, email: str = None, 
