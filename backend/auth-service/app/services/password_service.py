...
class PasswordService:
    ...
    async def verify_password(self, password: str, password_hash: str, 
                            user_id: str = None, ip_address: str = None) -> PasswordValidationResult:
        """验证密码"""
        try:
            # 预处理：添加pepper
            seasoned_password = password.encode() + self.pepper
            ...
            if is_valid and user_id:
                ...
                # 记录验证事件
                await self._log_password_verification(user_id, ip_address, is_valid, risk_factors)
            
            return PasswordValidationResult(
                is_valid=is_valid,
                breach_detected=breach_detected if 'breach_detected' in locals() else False,
                reuse_detected=reuse_detected if 'reuse_detected' in locals() else False,
                warnings=warnings,
    ...
    async def validate_password_strength(self, password: str, 
                                       user_context: Dict[str, Any] = None) -> PasswordStrengthResult:
        """验证密码强度"""
        feedback = []
        requirements_met = {}
        score = 0
        ...
        is_strong = (
            score >= 70 and 
            all(requirements_met.get(req, True) for req in ['length', 'uppercase', 'lowercase', 'numbers', 'special']
    ...
    @monitor_function("password")
    async def generate_secure_password(self, length: int = 16, 
    ...
    @monitor_function("password")
    async def check_password_expiry(self, user_id: str, 
                                  password_created_at: datetime) -> Dict[str, Any]:
        """检查密码是否过期"""
        try:
            # 密码有效期（90天）
            ...
            should_warn = age > (password_max_age - warning_threshold) and not is_expired
    ...
    async def _check_password_reuse(self, user_id: str, password: str) -> bool:
        ...
        try:
            ...
            for encrypted_hash in history:
                try:
                    # 解密历史密码哈希
                    decrypted_hash = self.fernet.decrypt(encrypted_hash.encode()).decode()
    ...
    async def _log_password_verification(self, user_id: str, ip_address: str, 
                                       is_valid: bool, risk_factors: List[str]):
        """记录密码验证事件"""
