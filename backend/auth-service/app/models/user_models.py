...
class TrustedDevice(Base):
    ...
    # 信任状态
    is_active = Column(Boolean, default=True, nullable=False)
    trust_level = Column(String(20), default="medium", nullable=False)  # low, medium, high
...
class PasswordHistory(Base):
    ...
    # 变更信息
    changed_by = Column(UUID(as_uuid=True), ForeignKey("users.id"), nullable=True)  # 谁修改的
...
User.security_events = relationship("SecurityEvent", back_populates="user", foreign_keys="SecurityEvent.user_id")
...
Index('idx_security_events_type_severity', SecurityEvent.event_type, SecurityEvent.severity)
