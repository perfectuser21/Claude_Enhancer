...
class SecurityMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        try:
            ...
            # 2. 速率限制
            if (settings.RATE_LIMIT_ENABLED and 
                request.url.path not in self.rate_limit_excluded_paths):
...
class CSRFProtection:
    ...
    async def protect_request(self, request: Request) -> bool:
        ...
        csrf_token = (
            ...
            (await request.form()).get('csrf_token') if hasattr(request, 'form') else None
        )
