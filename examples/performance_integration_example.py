...
# 创建FastAPI应用
app = FastAPI(
    title="Perfect21 Performance Demo API",
...
async def startup_event():
    ...
    performance_manager = await get_performance_manager(
        service_name="perfect21-demo-api",
    ...
    async with performance_manager.performance_context("process_user_data"):
        ...
        if performance_manager.async_processor:
            task_id = await performance_manager.async_processor.submit_custom_task(
                ...
                priority=performance_manager.async_processor.config.__class__.__dict__.get('TaskPriority', 'NORMAL')
            )
...
async def get_performance_stats():
    ...
    if performance_manager.metrics_collector:
        metrics_stats = await performance_manager.metrics_collector.get_metrics_summary()
...
async def health_check():
    ...
    if not performance_manager:
        return JSONResponse(
            status_code=503,
            content={"status": "unhealthy", "reason": "Performance manager not initialized"}
        )
...
async def simulate_load_request(request_id: int):
    ...
    # 模拟不同类型的操作
    operations = [
        lambda: performance_manager.cache_manager.set("load_test", f"key_{request_id}", f"value_{request_id}"),
        lambda: performance_manager.cache_manager.get("load_test", f"key_{request_id % 10}"),
...
async def run_demo():
    ...
    async with aiohttp.ClientSession() as session:
        ...
        # 4. 运行负载测试
        print("\n🔥 Phase 4: Load Test")
        async with session.post(f"{base_url}/api/load-test?requests_count=50") as response:
            data = await response.json()
